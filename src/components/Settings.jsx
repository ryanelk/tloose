...
export function SettingsMenu({ data, setData, saveData }) {
      ...
      {open && (
        <div style={{
          ...
          <div style={{ borderTop: "1px solid var(--border)", paddingTop: 12 }}>
            <button onClick={() => {
              if (confirm("Reset all data to sample template?")) { const fresh = JSON.parse(JSON.stringify(DEFAULT_DATA)); setData(fresh); saveData(fresh); }
              setOpen(false);
            }} style={{
